...
export function FacebookButton() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    Http.get<{ accessToken: string; refreshToken: string }>(`${url.oauth.facebook.callback}${window.location.search}`)
      .then(({ response, error }) => {
  ...
  return (
    <>
      <button
        ...
        onClick={handleClick}
      >
        {isLoading ? <Spinner className={"h-[18px] w-[18px]"} isDualRing={false} /> : <FacebookIcon className={"h-[18px] w-[18px]"} />}
